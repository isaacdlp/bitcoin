...
class BitcoinStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.smaShort = bt.ind.SimpleMovingAverage(self.BTC.close, period = 100)
        self.smaLong = bt.ind.SimpleMovingAverage(self.BTC.close, period = 400)
        self.vol = bt.ind.SimpleMovingAverage(self.BTC.volume, period = 80)
        self.last_id = 0
        super().__init__()
    ...
    def next(self):
        if len(self.broker.get_orders_open()) == 0.0:
            if self.smaLong[0] > self.smaShort[0] and self.vol[0] < self.BTC.volume[0]:
                self.do_exit(self.BTC)
            elif self.smaLong[0] < self.smaShort[0] and self.vol[0] < self.BTC.volume[0]:
